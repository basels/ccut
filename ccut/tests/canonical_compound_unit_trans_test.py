...
def test():
    ...
    a_oval, a_err = s.canonical_transform("km s^2", "hr^2 microft", 0.123)
    ...
    a_oval, a_err = s.canonical_transform("degC", "degF", 13.33)
    ...
    a_oval, a_err = s.canonical_transform("ha^2", "km^4", 3.049)
    e_oval, e_err = 0.0003049, 0
    assert a_err == e_err
    assert abs(a_oval - e_oval) < 0.00001

    # DIFFDIM #2 Single Unit Flow
    a_oval, a_err = s.canonical_transform("km^3", "mliter", 3.049)
    ...
    a_oval, a_err = s.canonical_transform("degC km", "K ft", 0.234)
    e_err = 1
    assert (a_err == e_err)

    # DIMENSION_MISMATCH Error flow
    a_oval, a_err = s.canonical_transform("km", "s", 124)
    e_err = 2
    assert (a_err == e_err)

    # TRANSFORMATION_UNKNOWN Error flow
    a_oval, a_err = s.canonical_transform("km s", "ft hr X", 267)
    e_err = 3
    assert (a_err == e_err)
    a_oval, a_err = s.canonical_transform("km s X", "ft hr", 7226)
    assert (a_err == e_err)